...
sigma = 2**7
print("sigma", sigma)
print("q", q, float(log(q, 2)))
print("logn", float(log(n, 2)))

# qmin = 0.0058*sigma**2*n**2.484
# print("qmin", log(qmin, 2))
# assert(q < qmin)
nb_queries = 2 ** 40

# ||p|| >= sqrt(Qs)*||T||*||c||
norm_noise_Tc = sqrt(nb_queries)*n*sigma*q2/2
normp = sqrt(norm_noise_Tc**2 + (norm_noise_Tc + sqrt(n)*q1/2)**2)
print("norm p", normp, float(log(normp, 2)))

# dans falcon, beta~1.17*sqrt(n)*sqrt(q) <= 1/5*q
assert(normp < q)

# pk size
pksize = n*ceil(log(q, 2))/8
print("pksize", pksize)

# sign size
entropygaussian = 1/2*n*(1+log(2*pi*normp**2, 2))/8

entropygaussian = 1.1*n*(1+log(sqrt(nb_queries)*n*sigma*q2/2/sqrt(n), 2))/8

signsize = 40 + entropygaussian #n*ceil(log(beta, 2))/8
print("signsize", float(signsize))
...
while(1):
    ...
    if left > right:
        ...
        B += 1
print("keyrec B", B, ceil(0.292*B), ceil(0.265*B))

# forgery
B = 100
sq = sqrt(q)
while ((B / (2 * pi * e)) ** (n / B)) * sq > normp:
    # print("debug", log(((B / (2 * pi * e)) ** (n / B)), 2))
    B += 1
print("forgery B", B, ceil(0.292*B), ceil(0.265*B))

def forgery_condition_mitaka(bkz_blocksize, max_sign_norm, gs_norm, n):
    B = bkz_blocksize
...
# B = 100
# sq = sqrt(q)
# while forgery_condition_mitaka(B, normp, sqrt(q), n):
#     B += 1
# print("forgery mitaka B", B, 0.292*B, 0.265*B)

B, _, _, _ = combined_attack_prob(q, n, sigma**2, ntru="matrix", fixed_tours=8, verbose=False, only="DSD")
print("keyrec overstreched", B, ceil(0.292*B), ceil(0.265*B))