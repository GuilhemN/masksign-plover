...
//  Plover public key
typedef struct {
    uint8_t a_seed[PLOVER_AS_SZ];             //  seed for a
    int64_t b[PLOVER_N];                      //  public key
...
typedef struct {
    ...
    int64_t s[PLOVER_D][PLOVER_N];              //  d-masked secret key
} plover_sk_t;

//  Plover signature
typedef struct {
    uint8_t salt[PLOVER_SALT_SZ];             //  Signature salt
    int64_t z[PLOVER_N];                      //  signature data
    int64_t c1[PLOVER_N];                     //  signature data
} plover_sig_t;
