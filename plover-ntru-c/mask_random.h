...
typedef struct {
    ...
    uint64_t s[PLOVER_D - 1][5];  //  Ascon state
#else
    uint64_t s[PLOVER_D - 1][2];  //  LFSR-127 state
#endif
} mask_random_t;
...
//  create a uniform random scalar mod q -- generator 0 <= ri < d-1
int64_t mask_random_scalar(mask_random_t *mrg, size_t ri);
